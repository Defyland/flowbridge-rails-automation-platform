...
- connector evidence masks credentials, signatures, cookies, and sensitive URL query parameters;
...
| Secret leakage in evidence | Stored webhook headers, connector request URLs, connector outputs, and connector errors mask sensitive keys and query parameters while preserving operational context. | `FlowBridge::SecretMasker`, `test/services/secret_masker_test.rb`, `test/services/node_executor_test.rb`, `test/integration/webhook_failure_scenarios_test.rb` |
...
```bash
bin/ci
bin/rails test test/services/http_egress_policy_test.rb test/services/secret_masker_test.rb test/services/node_executor_test.rb test/models/workflow_version_test.rb test/services/execution_runner_test.rb test/integration/rate_limiting_and_metrics_test.rb test/integration/openapi_response_contract_test.rb test/integration/webhook_failure_scenarios_test.rb test/repository_spec_compliance_test.rb
